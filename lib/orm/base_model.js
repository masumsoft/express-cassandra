...
var BaseModel = function(instance_values){
    ...
    var default_setter = function(prop_name, new_value){
        ...
        default_getter = function(prop_name){
            var retval;
            if(typeof fields[prop_name] != "undefined") {
                var fieldDataType;
                if(typeof fields[prop_name].type != "undefined") {
                    fieldDataType = fields[prop_name].type.toLowerCase();
                } else if(typeof fields[prop_name].toLowerCase != "undefined") {
                    fieldDataType = fields[prop_name].toLowerCase();
                }
                switch(fieldDataType) {
                    ...
                    case "double":
                    case "counter":
                        retval = parseInt(this[prop_name]);
                    break;
                    case "decimal":
                    case "float":
                        retval = parseFloat(this[prop_name]);
                    break;
                    case "boolean":
                        retval = Boolean(this[prop_name]);
                    break;
                    default:
                        retval = this[prop_name];
                    break;
                }
            } else {
                retval = this[prop_name];
    ...
    this.execute_query(query,[table_name,keyspace], function(err, result) {
        ...
        for(var r in result.rows){
            ...
            if(row.type == 'partition_key'){
                ...
                if(row.validator && row.validator.indexOf('org.apache.cassandra.db.marshal.ReversedType') == 0) {
...
BaseModel._get_db_value_expression = function(fieldname, fieldvalue){
    ...
    switch(fieldtype){
        ...
        case 'map':
            ...
            for(key in fieldvalue) {
                if(typeof fieldvalue[key] === "string"){
                    var typedef = this._properties.schema.fields[fieldname].typeDef.replace('<','').replace('>','').split(',');
                    if(typedef[0].trim().toLowerCase()=='timestamp' && (new Date(key)!='Invalid Date')) {
                        ...
                        retvalmap += "'"+key+"':'"+fieldvalue[key].replace(/'/g, "''")+"',";
                    }
                }
                    ...
                    retvalmap += "'"+key+"':"+fieldvalue[key].toString()+",";
                }
            }
        ...
        case 'list':
            ...
            for(key in fieldvalue) {
                if(typeof fieldvalue[key] === "string"){
                    retvallist += "'"+fieldvalue[key].replace(/'/g, "''")+"',";
                }
                else if(fieldvalue[key] instanceof Date) {
        ...
        case 'set':
            ...
            for(key in fieldvalue) {
                if(typeof fieldvalue[key] === "string"){
                    retvalset += "'"+fieldvalue[key].replace(/'/g, "''")+"',";
                }
                else if(fieldvalue[key] instanceof Date) {
                    retvallist += "'"+ fieldvalue[key].toISOString().slice(0,-5).replace('T',' ') + '+0000' +"',";
                }
                else {
                    retvalset += fieldvalue[key].toString()+",";
                }
            }
            //remove the final comma
...
BaseModel.prototype.delete = function(options, callback){
    ...
    for(var i in schema.key){
        ...
        delete_query[field_key] = field_value;
    }
