        ...
        if(model_schema.materialized_views) {
            ...
            for(var mvindex in model_schema.materialized_views) {
                ...
                for(var selectindex in model_schema.materialized_views[mvindex].select) {
                    if((typeof(model_schema.materialized_views[mvindex].select[selectindex]) != "string") || !(model_schema.materialized_views[mvindex].select[selectindex] in model_schema.fields))
                        throw("the select attribute under "+ mvindex +" of materialized_views must be an array of column name strings");

                    if(model_schema.fields[model_schema.materialized_views[mvindex].select[selectindex]].virtual)
