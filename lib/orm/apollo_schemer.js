...
var schemer = {
    ...
    normalize_model_schema: function(model_schema){
        ...
        if(output_schema.custom_index) {
            output_schema.custom_indexes = [output_schema.custom_index];
            delete output_schema.custom_index;
        }

        if(output_schema.custom_indexes) {
            var custom_array_sort = function(a,b) {
            ...
            output_schema.custom_indexes.sort(custom_array_sort);
        }
    ...
    validate_model_schema: function(model_schema){
        ...
        var validate_custom_index = function(custom_index) {
            if(typeof(custom_index) != "object")
                throw('custom_index must be an object with proper indexing attributes');
            if((typeof(custom_index.on) != "string") || !(custom_index.on in model_schema.fields))
                throw("custom_index must have an 'on' attribute with string value and value must be a valid column name");
            if(model_schema.fields[custom_index.on].virtual)
                throw("custom_index 'on' attribute must be a db column name, can't contain virtual field name");
            if(typeof(custom_index.using) != "string")
                throw("custom_index must have a 'using' attribute with string value");
            if(typeof(custom_index.options) != "object")
                throw("custom_index must have an 'options' attribute and it must be an object, pass blank {} object if no options are required");
        }

        if(model_schema.custom_index && model_schema.custom_indexes) {
            throw("both custom_index and custom_indexes are defined in schema, only one of them should be defined");
        }

        if(model_schema.custom_index) {
            validate_custom_index(model_schema.custom_index);
        }

        if(model_schema.custom_indexes) {
            if(model_schema.custom_indexes instanceof Array) {
                for(var ci=0; ci<model_schema.custom_indexes.length; ci++) {
                    validate_custom_index(model_schema.custom_indexes[ci]);
                }
            }
            else
                throw('custom_indexes must be an array with objects with proper indexing attributes');
        }
    },
